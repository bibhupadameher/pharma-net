...
function networkUp() {
  ...
  # Wait for 20 seconds to allow the docker network to stabilise
  sleep 1
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID transporter-sys-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then

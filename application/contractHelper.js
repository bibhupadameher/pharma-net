...
async function getContractInstance(organization, contractKey) {

	const uppercaseOrganization = organization.toUpperCase();

	// A gateway defines which peer is used to access Fabric network
	// It uses a common connection profile (CCP) to connect to a Fabric Peer
	// A CCP is defined manually in file connection-profile-organization.yaml
	gateway = new Gateway();
	...
	const fabricUserName = uppercaseOrganization + '_ADMIN';

	// Load connection profile; will be used to locate a gateway; The CCP is converted from YAML to JSON.
	let connectionProfile = yaml.safeLoad(fs.readFileSync('./connection-profile-' + organization + '.yaml', 'utf8'));
	...
	console.log('.....Connecting to Certnet Smart Contract');
		return channel.getContract('pharmanet', 'org.pharma-network.pharmanet.'+contractKey);
